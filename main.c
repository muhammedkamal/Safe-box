...
enum status { closed =0 ,setting=1 ,checking =2 ,open =3};
...
int main(void)
{
	enum status st = setting; // first setting the password
	LCD_init();
	keypad_init();
	...
	for(;;)
	{
		...
		LCD_command(0x81); /* lcd cursor location  to write open at center of the screen*/ 
		delayMs(500);
		LCD_data('W');
		LCD_data('R');
		LCD_data('I');
		...
		{
			LCD_command(1);	/* clear display */
			LCD_command(0x83); /* lcd cursor location  to write open at center of the screen*/ 
			delayMs(500);
			LCD_data('W');
			LCD_data('R');
			LCD_data('I');
			...
			if(check_pass()==1)
			{
				st= closed; // need more edits we have first to close the solonoid and we have to get sensor to check if open or closed
			}
			else 
			{
				LCD_command(1);	/* clear display */
		...
		{
			LCD_command(1);	/* clear display */
			LCD_command(0x85); /* lcd cursor location  to write open at center of the screen*/ 
			delayMs(500);
			LCD_data('C');
			LCD_data('L');
			LCD_data('O');
			...
			if(check_pass()==1)
			{
				st= open; // need more edits we have first to close the solonoid and we have to get sensor to check if open or closed
			}
...
void set_pass(void)
{
	...
	for (i=0;i<4;i++)
	{
		pass[i]= keypad_getchar(); //get password bits
...
unsigned char check_pass(void)
{
	...
	for (i=0;i<4;i++)
	{
		arr[i]= keypad_getchar(); //get password bits
		LCD_data('*'); //write * at the screen
	}
	for (i=0;i<4;i++)
	{
